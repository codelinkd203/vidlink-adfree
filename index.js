...
function findLocalChrome() {
  ...
  try {
    const found = execSync(
      'find /home/runner/.cache/puppeteer -name "chrome" -type f 2>/dev/null | head -1'
    ).toString().trim();
    if (found) return found;
  } catch { /* ignore */ }
  throw new Error('Chrome not found. Set CHROME_EXECUTABLE_PATH or run npm install with puppeteer first.');
}

const CHROME_PATH = findLocalChrome();
